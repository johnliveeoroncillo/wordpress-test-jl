...
define( 'CUSTOM_METABOXES_DIR', get_stylesheet_directory_uri() . '/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	if ( $repeatable ) {
		...
		$id = $repeatable[0] . '[' . $repeatable[1] . '][' . $id .']';
		$editor_id = str_replace("[","_",$repeatable[0] . '_' . $repeatable[1] . '_' . $id);
	...
	switch( $type ) {
		...
		case 'email':
		default:
			echo '<input ' . $required . ' type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . esc_attr( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		case 'url':
			echo '<input ' . $required . ' type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . esc_url( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		case 'number':
			echo '<input ' . $required . ' type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . intval( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		// textarea
		case 'textarea':
			echo '<textarea ' . $required . ' name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" cols="60" rows="4">' . esc_textarea( $meta ) . '</textarea>
					<br />' . $desc;
		break;
		// editor
		case 'editor':
			wp_enqueue_editor();
			$settings['textarea_name'] = $id;
			wp_editor( $meta, $editor_id, $settings ) . '<br />' . $desc;
		break;
		// checkbox
		case 'checkbox':
			echo '<input ' . $required . ' type="checkbox" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" ' . checked( $meta, true, false ) . ' value="1" />
					<label for="' . esc_attr( $id ) . '">' . $desc . '</label>';
		break;
		// select, chosen
		case 'select':
		case 'chosen':
			echo '<select ' . $required . ' name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"' , $type == 'chosen' ? ' class="chosen"' : '' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
		...
		case 'post_chosen':
			echo '<select data-placeholder="Select One" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"' , $type == 'post_chosen' ? ' class="chosen"' : '' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
					<option value="">No Parent</option>'; // Select One
			$posts = get_posts( array( 'post_type' => $post_type, 'posts_per_page' => -1, 'orderby' => 'name', 'order' => 'ASC' ) );
			foreach ( $posts as $item )
				echo '<option value="' . $item->ID . '"' . selected( $item->ID == $meta , true, false ) . '>' . $item->post_title . '</option>';
		...
		// date
		case 'date':
			echo '<input ' . $required . ' type="text" class="datepicker" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . $meta . '" size="30" />
					<br />' . $desc;
		break;
		// slider
		case 'slider':
		...
		case 'image':
			...
			echo	'<input ' . $required . ' name="' . esc_attr( $name ) . '" type="hidden" class="meta_box_upload_image" value="' . intval( $meta ) . '" />						
		...
		// repeatable
		case 'repeatable':
			echo '<table id="' . esc_attr( $id ) . '-repeatable" repeatable_name="' . esc_attr( $name ) . '" class="meta_box_repeatable" cellspacing="0" style="width:100%">
				<thead>
					<tr>
						<th><span class="sort_label"></span></th>
						<th>Fields</th>
						<th><a class="meta_box_repeatable_add" href="#">add</a></th>
					</tr>
				</thead>
				<tbody>';
			$i = 0;
			...
			foreach( $meta as $row ) {
				...
				echo '</td><td><a class="meta_box_repeatable_remove" href="#">remove</a></td></tr>';
				$i++;
			} // end each row
			echo '</tbody>';
			echo '
	...
	foreach( $meta as $key => $array ) {
		...
		foreach( $array as $arrayKey => $arrayValue ){
			...
			if ( is_array( $arrayValue ) ){
				//var_dump($arrayValue);
				//var_dump(unserialize($newSanitizer));
				
				$array[$arrayKey] = meta_box_array_map_r( $func, $arrayValue,$newSanitizer);
