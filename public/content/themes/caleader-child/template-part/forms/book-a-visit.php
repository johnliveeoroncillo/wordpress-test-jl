...
#book-visit-modal .modal-content {
    border-radius: 24px;
    min-width: 400px;
    max-width: 604px;
    overflow: hidden;
...
#book-visit-modal .modal-content .modal-body {
    text-align: center;
    padding: 32px 38px 50px 38px;
}

#book-visit-modal .modal-content .modal-body .card-title {
    font-family: 'Exo 2';
    font-style: normal;
    font-weight: 800;
    font-size: 40px;
...
#book-visit-modal .modal-content .modal-body p {
    font-weight: 300;
    font-size: 16px;
    color: #4F4F4F;
...
#book-visit-modal .modal-content .modal-body p.ref {
    font-weight: 700;
    color: #222222;
    font-size: 20px;
...
#book-visit-modal .modal-content .modal-body button {
    font-size: 16px;
    font-weight: 700;
    padding: 12px 0;
    width: 100%;
...
<div class="card">
    ...
    <div class="card-body">
        <form id="book-visit">
            <input type="hidden" name="car_slug" value="<?= get_query_var('book-car') ?>">
            <div class="row">
                <div class="col-12">
                ...
                <div class="col-12">
                    <div class="form-group text-center mb-0">
                        <button class="btn btn-lg btn-primary text-white btn-block" id="book-a-visit" type="submit" disabled>Book a Visit</button>
                    </div>
                </div>
...
<div class="modal fade" id="book-visit-modal">
    ...
    $(document).ready(function() {
        ...
        $("form#book-visit").validate({
            ...
            submitHandler: function(element) {
                ...
                $.ajax({
                    type: 'post',
                    url: '<?=CARSADA_URL .'/wp-json/rest-api/book-a-visit';?>',
                    dataType: 'json',
                    ...
                    success: function(data) {
                        $('.overlay').hide();
                        // console.log(data)
                        // console.log(data.id && data.book_reference_number);
                        if (data.id && data.book_reference_number) $('#book-visit-modal').show('bs.modal', function() {
                            $(this).addClass('show');
        ...
        $('[name="tnc"]').change(function() {            
            if($(this).is(":checked")) {
                $('#book-a-visit').prop("disabled", false);
            } else {
                $('#book-a-visit').prop("disabled", true)
            }
        });
        ...
        //     $('#book-visit-modal').show('bs.modal', function() {
        //         $(this).addClass('show');
        //     });
        // });
